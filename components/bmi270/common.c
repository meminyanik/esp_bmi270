...
static struct intf_config intf_conf;
...
{
    struct intf_config intf_info = *(struct intf_config *)intf_ptr;
    return I2Cdev::readBytes(intf_info.dev_addr, reg_addr, (uint8_t)len, reg_data);
}
...
{
    struct intf_config intf_info = *(struct intf_config *)intf_ptr;
    return (int8_t)I2Cdev::writeBytes(intf_info.dev_addr, reg_addr, (uint8_t)len, reg_data);
}
...
{
    vTaskDelay(period/portTICK_PERIOD_MS); // wait after reset
}
